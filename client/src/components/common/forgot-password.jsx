...
const styles = `
  ...
  .auth-card { width: 100%; max-width: 420px; background: var(--ems-bg-secondary, rgba(0,0,0,0.015)); border: 1px solid var(--ems-border, rgba(0,0,0,0.07)); border-radius: 24px; padding: 40px 36px; display: flex; flex-direction: column; gap: 28px; animation: fadeUp 0.6s ease both; }
  ...
  .auth-input { width: 100%; padding: 10px 14px; border-radius: 10px; border: 1px solid var(--ems-input-border, rgba(0,0,0,0.12)); background: var(--ems-input-bg, rgba(255,255,255,0.8)); font-size: 14px; font-family: 'DM Sans', sans-serif; color: #0f172a; outline: none; transition: border-color 0.2s, box-shadow 0.2s; box-sizing: border-box; }
  ...
  .auth-secondary-btn { width: 100%; padding: 11px; background: transparent; color: rgba(0,0,0,0.5); font-family: 'DM Sans', sans-serif; font-size: 14px; font-weight: 500; border: 1px solid rgba(0,0,0,0.1); border-radius: 10px; cursor: pointer; transition: background 0.2s, border-color 0.2s; letter-spacing: 0.02em; text-decoration: none; text-align: center; display: block; }
  .auth-secondary-btn:hover { background: rgba(0,0,0,0.04); border-color: rgba(0,0,0,0.15); }
                ...
                <div className="auth-body">
                    <div className="auth-card">
                        ...
                        <div>
                            ...
                            <p className="auth-subtitle">Enter your email address and we'll send you a reset link right away.</p>
                        </div>
                        ...
                        <form className="auth-form" onSubmit={handleforgotpasswordsubmit}>
                            <div className="auth-field">
                                ...
                                <input className="auth-input" id="email" name="email" type="email" required
                                    autoComplete="email" onChange={handlesforgotpasswordform}
                                    placeholder="you@company.com" />
                            </div>
                            <button type="submit" className="auth-submit-btn">Send reset email →</button>
                            <Link to={redirectpath} className="auth-secondary-btn">← Back to login</Link>
                        </form>
                        <p className="auth-footer-note">Reset links expire after 1 hour</p>
                    </div>
                </div>
            </div>
        </>

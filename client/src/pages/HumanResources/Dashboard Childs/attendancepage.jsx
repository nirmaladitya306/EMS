    ...
    return (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 mx-4 max-h-[80vh] flex flex-col">
                ...
                <p className="text-sm text-gray-500 mb-4">{record.employee?.firstname} {record.employee?.lastname}</p>
...
export const AttendancePage = () => {
    ...
    const handleDelete = (attendanceID) => {
        if (window.confirm('Delete this attendance record?')) dispatch(HandleDeleteAttendance({ attendanceID }))
    }
    ...
    return (
        <div className="attendance-page w-full mx-auto my-8 flex flex-col gap-6 h-[94%] pe-5">

            <div>
                <h1 className="text-3xl font-bold">Attendance</h1>
                <p className="text-sm text-gray-500 mt-1">View employee attendance records and daily logs</p>
            </div>
            ...
            <div className="flex flex-col gap-2 overflow-auto flex-1">
                ...
                {filtered.length === 0
                    ...
                    : filtered.map(a => (
                        <div key={a._id} className="grid grid-cols-6 bg-white border border-gray-200 rounded-lg px-4 py-3 text-sm items-center hover:bg-gray-50 transition-all">
                            <div className="col-span-2">
                                <p className="font-medium">{a.employee?.firstname} {a.employee?.lastname}</p>
                            </div>
                            <StatusBadge status={a.status} />
                            <span className="text-gray-600">{a.attendancelog?.length || 0} entries</span>
                            <span className="text-gray-500 text-xs">{fmtDate(a.updatedAt)}</span>
                            <div className="flex gap-2">
                                <button onClick={() => setLogTarget(a)}
